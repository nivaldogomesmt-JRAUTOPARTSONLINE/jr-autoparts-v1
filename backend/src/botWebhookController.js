...
async function trackingSupport(req, res) {
  try {
    ...
    try {
      ...
      if (normPlate) {
        contractInfo = await prisma.trackingContract.findFirst({
          where: { vehicle: { plate: normPlate } },
          select: {
            id: true,
            status: true,
            device: { select: { imei: true, model: true } },
    ...
    return ok(res, {
      ...
      message:    `Contrato de rastreamento localizado (${contractInfo.vehicle?.plate || normPlate || ''}). Um tÃ©cnico irÃ¡ verificar o dispositivo ${contractInfo.device?.imei || contractInfo.device?.model || ''}.`,
