...
function readXmlInput(req) {
  const fileText = req.file?.buffer ? Buffer.from(req.file.buffer).toString('utf8') : '';
  const bodyText = String(req.body?.xmlText || req.body?.xml || '').trim();
  return String(bodyText || fileText || '').replace(/^\uFEFF/, '').trim();
}

async function runXmlImport(xmlText, actor = 'Sistema') {
  if (!xmlText) {
    return { status: 400, body: { error: 'Envie um arquivo XML da NF-e ou cole o conteudo do XML.' } };
  }

  if (!xmlText.includes('<det')) {
    return { status: 400, body: { error: 'XML invalido: nenhum item <det> encontrado.' } };
  }

  const parsed = parseNfeProducts(xmlText);
  if (!parsed.length) {
    return { status: 400, body: { error: 'Nenhum item de produto encontrado no XML.' } };
  }
  ...
  await safeLogIntegration({
    area: 'Importacao XML',
    user: actor || 'Operacao Manual',
    quantity: result.created + result.updated,
    failures: result.ignored,
    reason: '-',
    meta: {
      items: parsed.length,
...
async function resolveInventoryProduct(tx, row) {
  const productId = String(row.id || row.productId || '').trim();
  const barcode = normalizeBarcode(row.barcode || row.code || row.sku || row.ean);
  const name = String(row.name || row.productName || row.description || '').trim();

  if (productId) {
    const byId = await tx.product.findUnique({ where: { id: productId } });
    if (byId) return byId;
  }

  if (barcode) {
    const byBarcode = await tx.product.findUnique({ where: { barcode } });
...
const importXml = async (req, res) => {
  try {
    const xmlText = readXmlInput(req);
    const response = await runXmlImport(xmlText, req.user?.name || req.user?.email || 'Sistema');
...
// POST /api/products/import/xml-text
const importXmlText = async (req, res) => {
  try {
    const xmlText = readXmlInput(req);
    const response = await runXmlImport(xmlText, req.user?.name || req.user?.email || 'Sistema');
    return res.status(response.status).json(response.body);
...
// POST /api/products/inventory/reconcile
const reconcileInventory = async (req, res) => {
  try {
    ...
    if (!payloadItems.length) {
      return res.status(400).json({ error: 'Envie ao menos um item para conferencia.' });
    }
    ...
    await prisma.$transaction(async (tx) => {
      for (const rawRow of payloadItems) {
        const counted = parseInventoryCount(rawRow?.countedStock ?? rawRow?.stock ?? rawRow?.quantity ?? rawRow?.count ?? rawRow?.actualStock);
        if (counted === null) {
          result.ignored += 1;
          result.rows.push({
            identifier: String(rawRow?.barcode || rawRow?.code || rawRow?.name || rawRow?.productId || '').trim(),
            status: 'IGNORED',
        ...
        const current = await resolveInventoryProduct(tx, rawRow);
        if (!current) {
          result.notFound += 1;
          result.rows.push({
            identifier: String(rawRow?.barcode || rawRow?.code || rawRow?.name || rawRow?.productId || '').trim(),
        ...
        const nextStock = mode === 'delta' ? Math.max(0, currentStock + counted) : counted;
    ...
    await safeLogIntegration({
      area: 'Conferencia de Estoque',
      user: req.user?.name || 'Operacao Manual',
      quantity: result.updated,
      failures: result.notFound + result.ignored,
      reason: result.notFound > 0 ? 'Alguns itens nao foram encontrados.' : '-',
      meta: {
        mode,
        submitted: payloadItems.length,
    ...
    return res.json({
      message: 'Conferencia de estoque concluida.',
      ...result,
      mode,
    });
  } catch (err) {
    return res.status(500).json({ error: 'Erro ao aplicar conferencia de estoque.' });
  }
};
...
module.exports = { list, overview, get, create, update, importXml, importXmlText, reconcileInventory, exportProducts, remove };
